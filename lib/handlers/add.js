import { nanoid } from "nanoid";
import books from "../books.js";

const addBooksHandler = (request, h) => {
    const {
        name,
        year,
        author,
        summary,
        publisher,
        pageCount,
        readPage,
        reading,
    } = request.payload;
    try {
        typeof name === "string"
            ? name
            : new Error("Gagal menambahkan buku. Mohon isi nama buku");
        typeof year === "number"
            ? year
            : new Error("Gagal menambahkan buku. Tahun harus berupa angka");
        typeof author === "string"
            ? author
            : new Error("Gagal menambahkan buku. Mohon isi nama penulis");
        typeof summary === "string"
            ? summary
            : new Error("Gagal menambahkan buku. Mohon isi ringkasan buku");
        typeof publisher === "string"
            ? publisher
            : new Error("Gagal menambahkan buku. Mohon isi nama penerbit");
        typeof pageCount === "number"
            ? pageCount
            : new Error(
                  "Gagal menambahkan buku. Jumlah halaman harus berupa angka"
              );
        typeof readPage === "number"
            ? readPage
            : new Error(
                  "Gagal menambahkan buku. Jumlah halaman yang sudah dibaca harus berupa angka"
              );
        typeof reading === "boolean"
            ? reading
            : new Error("Gagal menambahkan buku. Mohon isi status baca buku");
        if (readPage > pageCount) {
            throw new Error(
                "Gagal menambahkan buku. readPage tidak boleh lebih besar dari pageCount"
            );
        }
    } catch (error) {
        const response = h.response({
            status: "fail",
            message: error.message,
        });
        response.code(500);
        return response;
    }
    const id = nanoid(16);
    const insertedAt = new Date().toISOString();
    const updatedAt = insertedAt;

    const newBook = {
        id,
        name,
        year,
        author,
        summary,
        publisher,
        pageCount,
        readPage,
        finished: false,
        reading,
        insertedAt,
        updatedAt,
    };

    books.push(newBook);
    const Success = books.filter((book) => book.id === id).length > 0;
    if (Success) {
        const response = h.response({
            status: "success",
            message: "Buku berhasil ditambahkan",
            data: {
                bookId: id,
            },
        });
        response.code(201);
        return response;
    }
    const response = h.response({
        status: "fail",
        message: "Buku gagal ditambahkan",
    });
    response.code(500);
    return response;
};

// {
//     "name": string,
//     "year": number,
//     "author": string,
//     "summary": string,
//     "publisher": string,
//     "pageCount": number,
//     "readPage": number,
//     "reading": boolean
// }

export default addBooksHandler;
